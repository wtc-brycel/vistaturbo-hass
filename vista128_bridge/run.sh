...
cleanup_removed_options() {
  ...
  # Remove stale options that are no longer part of the supported App surface.
  for old_key in \
    ...
    if bashio::jq.exists "${options}" ".${old_key}"; then
      ...
      if ! bashio::addon.option "${old_key}"; then
        bashio::log.warning "Could not remove obsolete option '${old_key}'; continuing with supported defaults"
      fi
    fi
  done
}
...
export EVENT_HISTORY_STARTUP_DUMP_ENABLED="false"
...
export DIAGNOSTICS_SQLITE_PATH="/data/vistaturbo_diagnostics.sqlite3"
export DIAGNOSTICS_MAX_AGE_DAYS="30"
export DIAGNOSTICS_MAX_ROWS="25000"
export DIAGNOSTICS_HEALTH_SNAPSHOT_INTERVAL_SECONDS="900"
