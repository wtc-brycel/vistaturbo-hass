...
- `validate` is read-only (`contents: read`, `checks: read`, `actions: read`), checks out the exact `github.sha` without credential persistence, validates metadata, waits for the exact commit's newest successful GitHub Actions `test` and `frontend-render` checks, and uploads a one-day artifact named for that SHA.
