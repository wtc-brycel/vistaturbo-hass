(() => {
  ...
  template.innerHTML = `<style>${window.VISTA_MANAGEMENT_STYLES || ""}
    .log-app{overflow:hidden}.record-tabs{display:flex;min-height:48px;padding:0 10px;border-bottom:1px solid var(--vt-divider);background:var(--vt-card);overflow-x:auto}.record-tab{position:relative;min-height:48px;padding:0 14px;border:0;background:transparent;color:var(--vt-secondary);font:inherit;font-weight:500;cursor:pointer;white-space:nowrap}.record-tab:hover{background:var(--vt-hover);color:var(--vt-text)}.record-tab[aria-selected="true"]{color:var(--vt-primary)}.record-tab[aria-selected="true"]::after{content:"";position:absolute;left:10px;right:10px;bottom:0;height:3px;border-radius:3px 3px 0 0;background:var(--vt-primary)}.record-tab:focus-visible{outline:2px solid var(--vt-primary);outline-offset:-3px}.log-toolbar{display:flex;gap:8px;align-items:end;padding:10px 12px;flex-wrap:wrap}.compact-field{display:grid;gap:3px;color:var(--vt-secondary);font-size:11px}.compact-field select,.compact-field input{height:40px;min-width:128px;border:0;border-radius:8px;background:var(--vt-form);color:var(--vt-text);padding:0 10px}.advanced{border-top:1px solid var(--vt-divider)}.advanced summary{cursor:pointer;padding:9px 12px;color:var(--vt-primary);font-weight:500;list-style:none}.advanced summary::-webkit-details-marker{display:none}.advanced summary::before{content:"›";display:inline-block;margin-right:8px;transform:rotate(0deg);transition:transform .12s ease}.advanced[open] summary::before{transform:rotate(90deg)}.advanced-fields{display:grid;grid-template-columns:repeat(6,minmax(110px,1fr));gap:8px;padding:0 12px 10px}.table-scroll{overflow:auto;border-top:1px solid var(--vt-divider);max-height:650px}.data-table{width:100%;border-collapse:collapse;font-size:13px}.data-table th{position:sticky;top:0;background:var(--vt-surface-header,var(--vt-card));z-index:1;color:var(--vt-secondary);font-weight:500;text-align:left;white-space:nowrap;border-bottom:1px solid var(--vt-divider)}.data-table th,.data-table td{padding:9px 10px;border-bottom:1px solid var(--vt-divider)}.data-table th button{border:0;background:transparent;color:inherit;padding:0;cursor:pointer;font-weight:inherit}.data-table th.sorted{color:var(--vt-text)}.data-table th.sorted button::after{content:" ↓";color:var(--vt-primary)}.data-table th.sorted[data-dir="asc"] button::after{content:" ↑"}.data-table tbody tr{cursor:pointer}.data-table tbody tr:hover,.data-table tbody tr:focus-visible{background:var(--vt-hover);outline:none}.record-type{display:inline-flex;padding:2px 7px;border-radius:4px;font-size:11px;font-weight:600;background:var(--vt-chip);color:var(--vt-secondary)}.record-type.audit{color:var(--vt-primary);background:var(--vt-selected)}.mono{font-variant-numeric:tabular-nums;font-family:var(--code-font-family,ui-monospace,SFMono-Regular,Consolas,monospace)}.table-footer{display:flex;align-items:center;justify-content:flex-end;gap:8px;padding:8px 12px;color:var(--vt-secondary);font-size:12px}.table-footer label{display:flex;align-items:center;gap:5px}.table-footer select{height:32px;border:0;border-radius:6px;background:var(--vt-form);color:var(--vt-text)}.detail-kv{display:grid;grid-template-columns:150px minmax(0,1fr);gap:8px 14px;margin:0}.detail-kv dt{color:var(--vt-secondary)}.detail-kv dd{margin:0;overflow-wrap:anywhere}.loading{padding:12px;color:var(--vt-secondary)}@media(max-width:900px){.advanced-fields{grid-template-columns:repeat(3,1fr)}}@media(max-width:700px){.log-toolbar{align-items:stretch}.search-box{min-width:100%}.compact-field{flex:1}.compact-field select,.compact-field input{width:100%;min-width:0}.advanced-fields{grid-template-columns:1fr 1fr}.table-footer{flex-wrap:wrap}.log-table th:nth-child(2),.log-table td:nth-child(2){display:none}.detail-kv{grid-template-columns:1fr}.detail-kv dd{margin-bottom:8px}}@media(max-width:480px){.record-tab{padding-inline:11px}.advanced-fields{grid-template-columns:1fr}}
  </style>
  <section class="surface log-app">
    ...
    <div class="log-toolbar">
      ...
      <label class="compact-field">Partition<select id="partition"><option value="all">All</option></select></label>
      <label class="compact-field">Source / result<select id="source"><option value="all">All</option><option value="live">Live</option><option value="history">History</option><option value="both">Live + history</option><option value="queued">Queued</option><option value="accepted">Accepted</option><option value="confirmed">Confirmed</option><option value="acknowledged_unverified">Acknowledged unverified</option><option value="unverified">Unverified</option><option value="failed">Failed</option><option value="verification_mismatch">Verification mismatch</option><option value="rejected">Rejected</option><option value="timeout">Timeout</option></select></label>
    </div>
    <details class="advanced"><summary>Advanced filters</summary><div class="advanced-fields">
  ...
  <dialog id="detail-dialog"><div class="dialog-shell"><div class="dialog-head"><h3 id="detail-title"></h3><button class="icon-button" data-close type="button" aria-label="Close">✕</button></div><div class="dialog-scroll" id="detail-body"></div><div class="dialog-actions"><button class="ha-button" data-close type="button">Close</button></div></div></dialog>`;
  ...
  class VistaEventLogApp extends HTMLElement {
    ...
    _bind() {
      ...
      const schedule = () => { this._page = 1; this._refresh(); };
      const debounce = () => { clearTimeout(this._filterTimer); this._filterTimer = setTimeout(schedule, 180); };
    ...
    _populateFilters() {
      ...
      partition.innerHTML = '<option value="all">All</option>' + parts.map((item) => `<option value="${item.partition}">P${item.partition} ${esc(item.name || "")}</option>`).join("");
      ...
      if (!this._provider) {
        ...
        source.innerHTML = '<option value="all">All</option>' + sources.map((item) => `<option value="${esc(item)}">${esc(String(item).replaceAll("_", " "))}</option>`).join("");
    ...
    async _loadRemote() {
      ...
      this.shadowRoot.getElementById("rows").innerHTML = '<tr><td colspan="6" class="loading">Loading…</td></tr>';
      try {
        ...
        if (token !== this._loadToken) return;
        this.shadowRoot.getElementById("rows").innerHTML = `<tr><td colspan="6" class="empty">${esc(error?.message || "Event log unavailable")}</td></tr>`;
